function Jout = mvp_N_herm_adj_pwl_tucker(Jin,T)
        ...
        for l = 1:4
            ...
            for q = 1:3
                for lp = 1:4
                    ...
                    right = 4*(q-1) + lp;
                    NfJ  =  NfJ + sign(slp(l,lp)) * sign(llp(l,lp)) * conj(nmp(nmp(nmp(T(i,j).G,T(i,j).U1,1),T(i,j).U2,2),T(i,j).U3,3)) .* squeeze(fJ(:,:,:,right));
