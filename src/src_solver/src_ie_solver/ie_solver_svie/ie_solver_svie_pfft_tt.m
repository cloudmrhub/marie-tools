function[Jcb] = ie_solver_svie_pfft_tt(MREDM,gpu_flag)
    ...
    Jcb              = zeros(N_shield_sie+N_sie+N_b,A_ports);
    ...
    Multiplier_1 = to_GPU([ones(N_sie,1); -ones(N_b,1)],gpu_flag);
    Multiplier_2 = to_GPU([zeros(N_sie,1); ones(N_b,1)],gpu_flag);
    ...
    mvp          = @(J)mvp_svie_pfft_tt(J, pfft_PS, pfft_Z_cc, pfft_Z_bc_N, tt_Zbs_N, Zss, ZscU, ZscV, MREDM.EP.ext_Mc_inv, func, N, Multiplier_1, Multiplier_2, emc, N_shield_sie, N_sie, res, idx_solution, pfft_dims, nv, ql);
    for i = 1:A_ports
        tic
