function [MREDM] = pfft_proj_surface_create_near_lists(MREDM)    
    ...
    RWG_to_PWX_cells = zeros(pFFT_3Dkernel, N_sie);
    ...
    for i_sie = 1:N_sie
        ...
        RWG_to_PWX_cells(:,i_sie) = pfft_proj_find_expansion_cell(idx_x_ctr, idx_y_ctr,idx_z_ctr,pFFT_kernel,pfft_n1,pfft_n2,pfft_n3);
   
        % Find the indecies of the extended domain per RWG function that the projection has to be applied on
        C2Vox_near_list(:,i_sie)  = pfft_proj_get_near_indecies(idx_x_ctr,idx_y_ctr,idx_z_ctr,pfft_kernel_n1,pfft_n1,pfft_n2,pfft_n3);

        % Store centers of voxels per RWG function
        C2Exp_centers(:,i_sie)    = [idx_x_ctr, idx_y_ctr, idx_z_ctr];
        
        % Find the indecies of the expansion voxels that intersect with body voxels 
        C2B_near_vox = intersect(C2Vox_near_list(:,i_sie), pfft_idxS);
        if nnz(C2B_near_vox) > 0
            C2B_near_list(num2str(i_sie)) = C2B_near_vox;
        end
    ...
    for i_sie = 1:N_sie
        C2C_near_list(num2str(i_sie)) = find(max(abs(C2Exp_centers-C2Exp_centers(:,i_sie))) < pfft_near_dist);
    end
    ...
    MREDM.pfft.cell_id             = RWG_to_PWX_cells;
