function [MREDM] = pfft_surface_domain(MREDM)
    ...
    xc   = MREDM.SIE.coil.node(1,:);
    yc   = MREDM.SIE.coil.node(2,:);
    zc   = MREDM.SIE.coil.node(3,:);

    % Ln   = MREDM.SIE.coil.Ln;
    % Ln_all = Ln(:);
    % sorted_edges = sort(Ln_all, 'descend');
    % max_combined_length = sorted_edges(1) + sorted_edges(2);
    % voxels_required              = ceil(max_combined_length / res);
    % % Force voxel count to be ODD
    % if mod(voxels_required,2) == 0
    %     voxels_required = voxels_required + 1;
    % end
    
    pFFT_kernel                    = 3;%voxels_required;  
    ...
    d_near_triag = ceil(mean(triag_bounding_box));
    d_near       = ceil(1.6 * max(d_near_vox, d_near_triag));
