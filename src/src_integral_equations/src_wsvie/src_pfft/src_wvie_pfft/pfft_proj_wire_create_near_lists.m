function [MREDM] = pfft_proj_wire_create_near_lists(MREDM)    
    ...
    % Initialize
    C2Exp_centers    = zeros(3, N_wie);
    Tri_to_PWX_cells = zeros(pFFT_3Dkernel, N_wie);
    C2Vox_near_list  = zeros(pfft_kernel_n1*pfft_kernel_n2*pfft_kernel_n3, N_wie);
    ...
    for i_wie = 1:N_wie
        ...
        % Find nearest voxel to Triangle function center 
        r_wie = Triangle_cntr(i_wie,:).';
        [idx_x_ctr, idx_y_ctr, idx_z_ctr] = pfft_proj_find_nearest_voxel(r_wie, xd, yd, zd, res);
        
        % Find the voxels of the extended domain that Triangle functions have to expand to. 
        Tri_to_PWX_cells(:,i_wie) = pfft_proj_find_expansion_cell(idx_x_ctr, idx_y_ctr,idx_z_ctr,pFFT_kernel,pfft_n1,pfft_n2,pfft_n3);
   
        % Find the indecies of the extended domain per Triangle function that the projection has to be applied on
        C2Vox_near_list(:,i_wie)  = pfft_proj_get_near_indecies(idx_x_ctr,idx_y_ctr,idx_z_ctr,pfft_kernel_n1,pfft_n1,pfft_n2,pfft_n3);

        % Store centers of voxels per Triangle Function
        C2Exp_centers(:,i_wie)    = [idx_x_ctr, idx_y_ctr, idx_z_ctr];
        
        % Find the indecies of the expansion voxels that intersect with body voxels 
        C2B_near_vox = intersect(C2Vox_near_list(:,i_wie), pfft_idxS);
        if nnz(C2B_near_vox) > 0
            C2B_near_list(num2str(i_wie)) = C2B_near_vox;
        end
    ...
    for i_wie = 1:N_wie
        C2C_near_list(num2str(i_wie)) = find(max(abs(C2Exp_centers-C2Exp_centers(:,i_wie))) < pfft_near_dist);
    end
    ...
    MREDM.pfft.cell_id             = Tri_to_PWX_cells;
