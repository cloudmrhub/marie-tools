function [Zc2b_N,Zc2b_K] = pfft_wire_surface_assemble_direct_bc(MREDM)
    ...
    for key = C2B_near_list.keys
        ...
        if i_all <= N_wie
            rp_i = wire.F_point(i_all,:).';
            r2_i = wire.S_point(i_all,:).';
            rn_i = wire.T_point(i_all,:).';
            if ql == 3
            ...
            rp_i = coil.rp(:,i_all-N_wie);
            rn_i = coil.rn(:,i_all-N_wie);
            r2_i = coil.r2(:,i_all-N_wie);
            r3_i = coil.r3(:,i_all-N_wie);
            if ql == 3
        ...
        Zbc_Nop_val{i_all}     = Zbc_Nop_i;
        Zbc_Kop_val{i_all}     = Zbc_Kop_i;        
        I_idx{i_all}           = idx_B_dofs;
        J_idx{i_all}           = i_all * ones(size(Zbc_Nop_i));  
    end
    ...
    Zc2b_N = sparse(I_idx, J_idx, Zbc_Nop_val, N_b, N_wie+N_sie);
    Zc2b_K = sparse(I_idx, J_idx, Zbc_Kop_val, N_b, N_wie+N_sie);
