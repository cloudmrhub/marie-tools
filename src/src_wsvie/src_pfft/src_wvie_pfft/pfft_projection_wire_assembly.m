function [PS,MREDM] = pfft_projection_wire_assembly(MREDM)
    
    MREDM = pfft_proj_wire_create_near_lists(MREDM);
    wire  = MREDM.WIE.coil;
    k0    = MREDM.emc.k0;
    ce    = MREDM.emc.ce;
    ...
    [wp_vie, z_vie]  = gauss_1d(Quad_order_W2Wol);
    [wp_wie, z_wie]  = gauss_1d(Quad_order_wie);
    VIE_quads        = [Quad_order_W2Wol; wp_vie; z_vie];
    ...
    for i_wie = 1:N_wie
        ...
        ctr_vox_idx     = central_cell_id(:, i_wie);
        cel_vox_idx     = cell_id(:, i_wie);
        r_c             = squeeze(pfft_r(ctr_vox_idx(1), ctr_vox_idx(2), ctr_vox_idx(3),:));
        cur_col_points  = col_points + r_c; 
        
        rp       = wire.F_point(i_wie,:).';
        r2       = wire.S_point(i_wie,:).';
        rn       = wire.T_point(i_wie,:).';
    ...
    P     = sparse(I_val, (1:N_wie) .* ones(N_cells * PWX_sz, 1), Val, Next_dofs, N_wie);
