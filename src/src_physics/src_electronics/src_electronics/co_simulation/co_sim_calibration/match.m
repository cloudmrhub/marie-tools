function [SPm,ZPm,ZPm_loss] = match(YPm,...
    ...
    for i = 1:length_matching_network
        ...
        if sum(is_capacitor_parallel_local)> 0 || sum(is_inductor_parallel_local)>0 || sum(is_resistor_parallel_local)>0
            ...
            if ~isempty(E_cp)
                YPm(logical(is_capacitor_parallel_local),logical(is_capacitor_parallel_local)) = YPm(logical(is_capacitor_parallel_local),logical(is_capacitor_parallel_local)) + diag(1./(E_cp+Cap_loss));
                YPm_loss(logical(is_capacitor_parallel_local),logical(is_capacitor_parallel_local)) = YPm_loss(logical(is_capacitor_parallel_local),logical(is_capacitor_parallel_local)) + diag(1./E_cp);
            end
            if ~isempty(E_lp)
                YPm(logical(is_inductor_parallel_local),logical(is_inductor_parallel_local)) = YPm(logical(is_inductor_parallel_local),logical(is_inductor_parallel_local)) + diag(1./(E_lp+Ind_loss));
                YPm_loss(logical(is_inductor_parallel_local),logical(is_inductor_parallel_local)) = YPm_loss(logical(is_inductor_parallel_local),logical(is_inductor_parallel_local)) + diag(1./E_lp);
            end
            if ~isempty(E_rp)
                YPm(logical(is_resistor_parallel_local),logical(is_resistor_parallel_local)) = YPm(logical(is_resistor_parallel_local),logical(is_resistor_parallel_local)) + diag(1./E_rp);
            end
        end
        if sum(is_capacitor_series_local)> 0 || sum(is_inductor_series_local)>0 || sum(is_resistor_series_local)>0
            ...
            if ~isempty(E_cs)
                ZPm(logical(is_capacitor_series_local),logical(is_capacitor_series_local)) = ZPm(logical(is_capacitor_series_local),logical(is_capacitor_series_local)) + diag(E_cs + Cap_loss);
                ZPm_loss(logical(is_capacitor_series_local),logical(is_capacitor_series_local)) = ZPm_loss(logical(is_capacitor_series_local),logical(is_capacitor_series_local)) + diag(E_cs);
            end
            if ~isempty(E_ls)
                ZPm(logical(is_inductor_series_local),logical(is_inductor_series_local)) = ZPm(logical(is_inductor_series_local),logical(is_inductor_series_local)) + diag(E_ls+Ind_loss);
                ZPm_loss(logical(is_inductor_series_local),logical(is_inductor_series_local)) = ZPm_loss(logical(is_inductor_series_local),logical(is_inductor_series_local)) + diag(E_ls);
            end
            if ~isempty(E_rs)
                ZPm(logical(is_resistor_series_local),logical(is_resistor_series_local)) = ZPm(logical(is_resistor_series_local),logical(is_resistor_series_local)) + diag(E_rs);
            end
            YPm = inv(ZPm);
