function[h_field] = em_hfield_wvie_pfft_tt(MREDM,gpu_flag)
    ...
    for i = 1:2
        ...
        for port = 1:A_ports
            ...
            H_vox_inc                 = pfft_Z_bc_K * Jc(:,port) +  pfft_PS(:,N_wie+1:end).' * H_vox_inc(:);
            H_vox_sca                 = pfft_PS(:,N_wie+1:end).' * H_vox_sca(:);
            tested_htot               = zeros(n1,n2,n3,ql);
            tested_hsca               = zeros(n1,n2,n3,ql);
            tested_hinc               = zeros(n1,n2,n3,ql);
            tested_hinc_shield        = func.mvp_Zbs(MREDM.operators.tt_Zbs_K,Js(:,port));
            tested_hsca(idx_solution) = H_vox_sca(:);
            tested_hinc(idx_solution) = H_vox_inc(:) + tested_hinc_shield(idx_solution);
