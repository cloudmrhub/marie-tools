function[e_field] = em_efield_wvie_pfft(MREDM,gpu_flag)
   ...
   % Compute
    for i = 1:2
        ...
        for port = 1:A_ports
            ...
            Jinc                      = reshape(pfft_PS(:,1:N_wie)*Jc(:,port),pfft_dims);
            Jsca                      = reshape(pfft_PS(:,N_wie+1:end)*Jb(:,port),pfft_dims);
            ...
            E_vox_inc                 = pfft_Z_bc_N * Jc(:,port) +  pfft_PS(:,N_wie+1:end).' * E_vox_inc(:);
            E_vox_sca                 = pfft_PS(:,N_wie+1:end).' * E_vox_sca(:);
            ...
            tested_einc(idx_solution) = E_vox_inc(:);
        ...
        for port = 1:A_ports 
            phi_coil(port,port) = 1/2 * Jc(:,port)' * (MREDM.WIE.Z_copper_loss * Jc(:,port));
            phi_lump(port,port) = 1/2 * Jc(:,port)' * (MREDM.WIE.Z_lumped_loss * Jc(:,port));
        end
