function[e_field] = em_efield_shield_only_svie(MREDM,gpu_flag)
    ...
    res                   = dims.res;
    idxS                  = dims.idxS;
    max_rank_N_shield_sie = dims.max_rank_N_shield_sie;
    ...
    for i = 1:2
        ...
        for port = 1:A_ports
            tested_einc           = func.mvp_Zbc(MREDM.operators.Zbs_N,Js(:,port),n1,n2,n3,N_shield_sie,max_rank_N_shield_sie); 
        ...
        for port = 1:A_ports 
            phi_coil(port,port) = 1/2 * Js(:,port)' * (MREDM.SIE.Z_shield_copper_loss * Js(:,port));
            phi_lump(port,port) = 1/2 * Js(:,port)' * (MREDM.SIE.Z_shield_lumped_loss * Js(:,port));
        end
