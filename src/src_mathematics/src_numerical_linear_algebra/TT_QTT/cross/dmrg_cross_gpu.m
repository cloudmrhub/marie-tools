function [y]=dmrg_cross_gpu(d,n,fun,eps)
    ...
    while ( swp < nswp && not_converged )
        ...
        s=diag(s); 
        r=gather(my_chop2(s,norm(s)*eps/sqrt(d-1))); 
...
%             fprintf('Cond(u) %f: \n', cond(u));
            ...
            ind_new=zeros(ry(i+1),i,'like',ind);
            for s=1:ry(i+1)
                f_in=ind(s);
...
%             fprintf('Cond(v) %f: \n', cond(v));
            ...
            r1=v(ind,:);
            v=v/r1; 
            v2=reshape(v,[n(i+1),ry(i+2),ry(i+1)]); y{i+1}=permute(v2,[3,1,2]);
            r1=r1*rm; r1=r1.';
            u=u*r1; y{i}=reshape(u,[ry(i),n(i),ry(i+1)]);
            v=v.'; 
            ...
            ind_new=zeros(d-i,ry(i+1),'like',ind);
            for s=1:ry(i+1)
                f_in=ind(s);
            ...
            if ( i == 1 ) 
                ...
                if ( er_max < eps ) 
                    not_converged=false;
                    fprintf('\n\t\t\t\tConvergence error: %4.6f %%',er_max*100);
                else
                    fprintf('\n\t\t\t\tConvergence error: %4.6f %%',er_max*100);
                    er_max=0;
                end
